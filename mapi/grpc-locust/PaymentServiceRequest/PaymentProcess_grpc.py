...
from gateway.requests.PaymentProcessRequest_pb2 import PaymentProcessRequest
from gateway.responses.PaymentProcessResponse_pb2 import PaymentProcessResponse
...
class TesterClient:
    ...
    def payment_process(self, request: PaymentProcessRequest) -> PaymentProcessResponse:
        stub = PaymentServiceStub(grpc.intercept_channel(grpc.insecure_channel(self.host), *self.interceptors))
        stub.paymentProcess(request=request)
...
class PerfTaskSet(TaskSet):
    ...
    def payment_process(self):
        req_data = PaymentProcessRequest(deviceInfo=self.get_device_info(), appInfo=self.get_app_info(), paymentMode=4,
                                         paymentType=0, shipId="", bankCode="")
        self.locust_request_handler("payment_process", req_data)
    ...
    def _get_request_function(self, grpc_name):
        req_func_map = {
            "payment_process": self.client.payment_process
        }
